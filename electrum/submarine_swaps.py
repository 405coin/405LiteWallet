...
MIN_FINAL_CLTV_DELTA_FOR_CLIENT = 3 * 144                                                                      
...
def _check_swap_scriptcode(
    *,
    ...
    payment_hash: bytes,
    locktime: int,
    refund_pubkey: Optional[bytes],                                                         
    claim_pubkey: Optional[bytes],                                      
) -> None:
...
class SwapManager(Logger):
    ...
    def __init__(self, *, wallet: 'Abstract_Wallet', lnworker: 'LNWallet'):
        ...
        self._swaps = self.wallet.db.get_dict('submarine_swaps')                             
...
class NostrTransport(SwapServerTransport):
    ...
    def __init__(self, config, sm, keypair: Keypair):
        ...
        self.taskgroup = OldTaskGroup()
        self._last_swapserver_relays = self._load_last_swapserver_relays()                                 
        self._swap_server_requests = asyncio.Queue(maxsize=5)                             
    ...
    def _store_last_swapserver_relays(self, relays: Sequence[str]):
        ...
        try:
            with open(storage_path, 'w', encoding="utf-8") as f:
                json.dump(relays, f, indent=4, sort_keys=True)                
        except Exception:
